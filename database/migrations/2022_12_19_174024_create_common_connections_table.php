...
return new class extends Migration
{
    ...
    public function up()
    {
        Schema::create('common_connections', function (Blueprint $table) {
            $table->id();
            $table->foreignId('user1_id')->references('id')->on('users')->constrained()->onDelete('cascade');
            $table->foreignId('user2_id')->references('id')->on('users')->constrained()->onDelete('cascade');
